...
namespace Turingforge {
    ...
    struct Equal {
        ...
        inline auto operator()(R1&& r1, R2&& r2, Turingforge::Scalar eps = 0.0) const noexcept -> bool
        {
    ...
    struct ParetoDominance {
        ...
        template<std::ranges::forward_range R1, std::ranges::forward_range R2>
        inline auto operator()(R1&& r1, R2&& r2, Turingforge::Scalar eps) const noexcept -> Dominance
        {
            return (*this)(std::begin(r1), std::end(r1), std::begin(r2), std::end(r2), eps);
        }
    };
