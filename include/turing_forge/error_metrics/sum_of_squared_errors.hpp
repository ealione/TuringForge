...
template<typename T>
inline auto SumOfSquaredErrors(Turingforge::Span<T const> x, Turingforge::Span<T const> y) noexcept -> double
{
...
template<typename T>
inline auto SumOfSquaredErrors(Turingforge::Span<T const> x, Turingforge::Span<T const> y, Turingforge::Span<T const> w) noexcept -> double
{
    static_assert(std::is_arithmetic_v<T>, "T must be an arithmetic type.");
    EXPECT(x.size() == y.size());
    EXPECT(!x.empty());
