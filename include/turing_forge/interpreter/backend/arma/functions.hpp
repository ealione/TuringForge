...
namespace Turingforge::Backend {
    ...
    auto Map(T* ptr) {
        return arma::Mat<Turingforge::Scalar>(ptr, S, 1, /*copy_aux_mem =*/false, /*strict=*/true);
    }

    template<typename T, std::size_t S>
    auto Map(T const* ptr) {
        using U = std::remove_const_t<T>;
        return arma::Mat<Turingforge::Scalar>(const_cast<U*>(ptr), S, 1, /*copy_aux_mem =*/false, /*strict=*/true);
    }
