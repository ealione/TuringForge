...
namespace Turingforge::Backend {
    namespace detail::fast_approx {
        ...
        // unary details
        inline auto constexpr Inv(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::InvImpl<Precision>(x);
        }

        inline auto constexpr Log(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::LogImpl<Precision>(x);
        }

        inline auto constexpr Log1p(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::Log1pImpl<Precision>(x);
        }

        inline auto constexpr Logabs(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::LogabsImpl<Precision>(x);
        }

        inline auto constexpr Exp(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::ExpImpl<Precision>(x);
        }

        inline auto constexpr Sin(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::SinImpl<Precision>(x);
        }

        inline auto constexpr Cos(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::CosImpl<Precision>(x);
        }

        inline auto constexpr Tan(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::TanImpl<Precision>(x);
        }

        inline auto constexpr Sinh(Turingforge::Scalar x) -> Turingforge::Scalar {
            auto const e = Exp(x);
            return (e*e - 1.F) * Inv(e+e);
        }

        inline auto constexpr Cosh(Turingforge::Scalar x) -> Turingforge::Scalar {
            auto const e = Exp(x);
            return (e*e + 1.F) * Inv(e+e);
        }

        inline auto constexpr ISqrt(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::ISqrtImpl<Precision>(x);
        }

        inline auto constexpr Sqrt(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::SqrtImpl<Precision>(x);
        }

        inline auto constexpr Sqrtabs(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::SqrtabsImpl<Precision>(x);
        }

        inline auto constexpr Div(Turingforge::Scalar x, Turingforge::Scalar y) -> Turingforge::Scalar {
            return fast_approx::DivImpl<Precision>(x, y);
        }

        inline auto constexpr Pow(Turingforge::Scalar x, Turingforge::Scalar y) -> Turingforge::Scalar {
            return fast_approx::PowImpl<Precision>(x, y);
        }

        inline auto constexpr Tanh(Turingforge::Scalar x) -> Turingforge::Scalar {
            return fast_approx::TanhImpl<Precision>(x);
        }

        inline auto constexpr Aq(Turingforge::Scalar x, Turingforge::Scalar y) -> Turingforge::Scalar {
