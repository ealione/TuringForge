...
namespace Turingforge::Backend::detail::fast_approx {
    ...
    inline auto constexpr CosImpl(Turingforge::Scalar x) -> Turingforge::Scalar {
    ...
    template<std::size_t P = 0>
    inline auto constexpr SinImpl(Turingforge::Scalar x) -> Turingforge::Scalar {
        if (!std::isfinite(x)) { return std::numeric_limits<Turingforge::Scalar>::quiet_NaN(); }
        if (x == 0) { return x; }
        if constexpr (P == 0) {
            auto constexpr invPi = std::numbers::inv_pi_v<float>;
    ...
    template<std::size_t P = 0>
    inline auto constexpr TanImpl(Turingforge::Scalar x) -> Turingforge::Scalar {
        if (x == 0) { return x; }
