...
namespace Turingforge {
    ...
    struct Dispatch {
        ...
        using Callable = std::function<void(Turingforge::Vector<Function> const&, A&, size_t, Turingforge::Range)>;

        template<concepts::ArrayBase A>
        using CallableDiff = std::function<void(Turingforge::Vector<Function> const&, A const&, A&, int, int)>;
        ...
        template<FunctionType Type, concepts::ArrayBase A>
        requires Function::IsNary<Type>
        static inline void NaryOp(Turingforge::Vector<Function> const& nodes, A& m, size_t parentIndex, Turingforge::Range /*unused*/)
        {
        ...
        static inline void BinaryOp(Turingforge::Vector<Function> const& nodes, A& m, size_t i, Turingforge::Range /*unused*/)
        {
            using R = Eigen::Ref<decltype(m.col(0))>;
            auto j = i - 1;
        ...
        static inline void UnaryOp(Turingforge::Vector<Function> const& /*unused*/, A& m, size_t i, Turingforge::Range /*unused*/)
        {
            using R = Eigen::Ref<decltype(m.col(0))>;
        ...
        template<FunctionType Type, concepts::ArrayBase A>
        static inline void DiffOp(Turingforge::Vector<Function> const& nodes, A const& primal, A& trace, int i, int j) {
...
//        }(std::make_index_sequence<sizeof...(Ts)-1>{})), Tup>;
        ...
        using Typ = decltype([]<auto... Idx>(std::index_sequence<Idx...>){
            return std::make_tuple(std::tuple_element_t<Idx, Tup>{}...);
    ...
    public:
        ...
        explicit DispatchTable(std::unordered_map<Turingforge::Hash, Tuple> const& map) : map_(map.begin(), map.end()) { }
        ...
        template<typename T>
        inline auto GetFunction(Turingforge::Hash const h) -> Dispatch::Callable<Array<T>>&
        {
            return const_cast<Dispatch::Callable<T>&>(const_cast<DispatchTable<Ts...> const*>(*this)->GetFunction(h)); // NOLINT
        }

        template<typename T>
        inline auto GetDerivative(Turingforge::Hash const h) -> Dispatch::CallableDiff<Array<T>>&
        {
            return const_cast<Dispatch::CallableDiff<T>&>(const_cast<DispatchTable<Ts...> const*>(*this)->GetDerivative(h)); // NOLINT
        }

        template<typename T>
        [[nodiscard]] inline auto GetFunction(Turingforge::Hash const h) const -> Dispatch::Callable<Array<T>> const&
        {
        ...
        template<typename T>
        [[nodiscard]] inline auto GetDerivative(Turingforge::Hash const h) const -> Dispatch::CallableDiff<Array<T>> const&
        {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        [[nodiscard]] inline auto Get(Turingforge::Hash const h) const -> std::tuple<Dispatch::Callable<A>, Dispatch::CallableDiff<A>>
        {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        template<typename T>
        [[nodiscard]] inline auto TryGetFunction(Turingforge::Hash const h) const noexcept -> std::optional<Dispatch::Callable<Array<T>>>
        {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        template<typename T>
        [[nodiscard]] inline auto TryGetDerivative(Turingforge::Hash const h) const noexcept -> std::optional<Dispatch::CallableDiff<Array<T>>>
        {
            if (auto it = map_.find(h); it != map_.end()) {
