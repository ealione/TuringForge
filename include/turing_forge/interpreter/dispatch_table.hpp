...
#include "turing_forge/core/range.hpp"
...
namespace Turingforge {
    ...
    struct Dispatch {
        ...
        template<typename T, std::size_t S> using Callable = std::function<void(Turingforge::Vector<Function> const &,
                                                                                Backend::View<T, S>, size_t,
                                                                                Turingforge::Range)>;

        template<typename T, std::size_t S> using CallableDiff = std::function<void(
                Turingforge::Vector<Function> const &, Backend::View<T const, S>, Backend::View<T, S>, int, int)>;
        ...
        template<FunctionType Type, typename T, std::size_t S>
        requires Function::IsNary<Type>
        static inline void
        NaryOp(Turingforge::Vector<Function> const &functions, Backend::View<T, S> data, size_t parentIndex, Turingforge::Range /*unused*/) {
            static_assert(Type < FunctionType::Aq);
            const auto nextArg = [&](size_t i) { return i - (functions[i].Length + 1); };
            ...
            int arity = functions[parentIndex].Arity;
            while (arity > 0) {
                switch (arity) {
                    case 1: {
        ...
        template<FunctionType Type, typename T, std::size_t S>
        requires Function::IsBinary<Type>
        static inline void BinaryOp(Turingforge::Vector<Function> const &nodes, Backend::View<T, S> m, size_t i, Turingforge::Range /*unused*/) {
            auto j = i - 1;
        ...
        template<FunctionType Type, typename T, std::size_t S>
        requires Function::IsUnary<Type>
        static inline void UnaryOp(Turingforge::Vector<Function> const & /*unused*/, Backend::View<T, S> m, size_t i, Turingforge::Range /*unused*/) {
        ...
        template<FunctionType Type, typename T, std::size_t S>
        static inline void DiffOp(Turingforge::Vector<Function> const &nodes, Backend::View<T const, S> primal, Backend::View<T, S> trace, int i, int j) {
            Diff<T, Type, S>{}(nodes, primal, trace, i, j);
        }

        template<FunctionType Type, typename T, std::size_t S>
        static constexpr auto MakeFunctionCall() -> Dispatch::Callable<T, S> {
            if constexpr (Function::IsNary<Type>) {
                return Callable<T, S>{NaryOp<Type, T, S>};
            } else if constexpr (Function::IsBinary<Type>) {
                return Callable<T, S>{BinaryOp<Type, T, S>};
            } else if constexpr (Function::IsUnary<Type>) {
    ...
    namespace detail {
        ...
        template<typename T> concept ExtentsLike = requires {
            { T::size() };
            { std::is_array_v<T> };
        };
    ...
    private:
        ...
        template<typename T, auto SZ = std::tuple_size_v<Typ>> requires (detail::TypeIndexImpl<T, Ts...>() < SZ)
    ...
    public:
        ...
        template<typename T> static constexpr typename Ext::value_type BatchSize = Sizes[TypeIndex<T>];

        template<typename T> using Backend = Backend::View<T, BatchSize<T>>;

        template<typename T> using Callable = Dispatch::Callable<T, BatchSize<T>>;

        template<typename T> using CallableDiff = Dispatch::CallableDiff<T, BatchSize<T>>;

    private:
        template<FunctionType Type, typename T>
        ...
        static constexpr auto MakeTuple() {
            return []<auto... Idx>(std::index_sequence<Idx...>) {
                return std::make_tuple(std::make_tuple(MakeFunction<Type, std::tuple_element_t<Idx, Tup>>()...),
        ...
        using Map = Turingforge::Map<Turingforge::Hash, Tuple>;

        Map map_;

    public:
        DispatchTable() {
            auto constexpr f = [](auto i) { return static_cast<FunctionType>(1U << i); };
            [&]<auto ...I>(std::index_sequence<I...>) {
                (map_.insert({Function(f(I)).HashValue, MakeTuple<f(I)>()}), ...);
            }(std::make_index_sequence<FunctionTypes::Count - 3>{});
        }
        ...
        template<typename U> static constexpr auto SupportsType = TypeIndex<U> < std::tuple_size_v<Typ>;

        explicit DispatchTable(Map const &map) : map_(map) {}

        explicit DispatchTable(Map &&map) : map_(std::move(map)) {}

        explicit DispatchTable(std::unordered_map<Turingforge::Hash, Tuple> const &map) : map_(map.begin(),
                                                                                               map.end()) {}
        ...
        template<typename T>
        inline auto GetFunction(Turingforge::Hash const h) -> Callable<T> & {
            return const_cast<Callable<T> &>(const_cast<DispatchTable<Ts...> const *>(*this)->GetFunction(h)); // NOLINT
        }

        template<typename T>
        inline auto GetDerivative(Turingforge::Hash const h) -> CallableDiff<T> & {
            return const_cast<CallableDiff<T> &>(const_cast<DispatchTable<Ts...> const *>(*this)->GetDerivative(h)); // NOLINT
        }

        template<typename T>
        [[nodiscard]] inline auto GetFunction(Turingforge::Hash const h) const -> Callable<T> const & {
        ...
        template<typename T>
        [[nodiscard]] inline auto GetDerivative(Turingforge::Hash const h) const -> CallableDiff<T> const & {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        template<typename T>
        [[nodiscard]] inline auto Get(Turingforge::Hash const h) const -> std::tuple<Callable<T>, CallableDiff<T>> {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        template<typename F>
        void RegisterCallable(Turingforge::Hash hash, F &&f) {
            map_[hash] = MakeTuple<F, Ts...>(std::forward<F &&>(f), Dispatch::Noop{});
        }

        template<typename F, typename DF>
        void RegisterCallable(Turingforge::Hash hash, F &&f, DF &&df) {
            map_[hash] = MakeTuple<F, Ts...>(std::forward<F &&>(f), std::forward<DF &&>(df));
        }

        template<typename T>
        [[nodiscard]] inline auto
        TryGetFunction(Turingforge::Hash const h) const noexcept -> std::optional<Callable<T>> {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        template<typename T>
        [[nodiscard]] inline auto
        TryGetDerivative(Turingforge::Hash const h) const noexcept -> std::optional<CallableDiff<T>> {
            if (auto it = map_.find(h); it != map_.end()) {
        ...
        [[nodiscard]] auto Contains(Turingforge::Hash hash) const noexcept -> bool { return map_.contains(hash); }
    }; // struct DispatchTable

    using DefaultDispatch = DispatchTable<Turingforge::Scalar>;
} // namespace Turingforge