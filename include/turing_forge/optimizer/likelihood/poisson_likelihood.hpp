...
template<typename T = Turingforge::Scalar, bool LogInput = true>
struct PoissonLikelihood : public LikelihoodBase<T> {

    PoissonLikelihood(Turingforge::RandomGenerator& rng, InterpreterBase<T> const& interpreter, Turingforge::Span<Turingforge::Scalar const> target, Turingforge::Range const range, std::size_t const batchSize = 0)
    ...
    auto operator()(Cref x, Ref g) const noexcept -> Turingforge::Scalar final {
        ++feval_;
    ...
    static auto ComputeLikelihood(Span<Scalar const> x, Span<Scalar const> y, Span<Scalar const> w) -> Scalar {
        ...
        vstat::univariate_accumulator<Turingforge::Scalar> acc;
...
private:
    auto SelectRandomRange() const -> Turingforge::Range {
        ...
        return Turingforge::Range{range_.Start() + s, range_.Start() + s + batchSize_};
    }
