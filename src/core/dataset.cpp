...
// internal implementation details
    ...
    auto Dataset::ReadCsv(std::string const &path, bool hasHeader) -> Dataset::Matrix {
        std::ifstream f(path);
        Turingforge::CSV::CsvParser parser(f);
        ...
        if (hasHeader) {
            ...
            for (auto const &row: parser) {
                for (auto const &_f: row) {
                    auto h = hash(_f);
                    variables_.insert({h, {_f, h, ncol++}});
                }
    ...
    Dataset::Dataset(std::vector<std::vector<Turingforge::Scalar>> const &vals)
            : variables_(DefaultVariables(vals.size())), values_(MatrixFromValues(vals)),
              map_(values_.data(), values_.rows(), values_.cols()) {
    }

    Dataset::Dataset(std::string const &path, bool hasHeader)
            : values_(ReadCsv(path, hasHeader)), map_(values_.data(), values_.rows(), values_.cols()) {
    }

    Dataset::Dataset(Matrix vals)
            : variables_(DefaultVariables(static_cast<size_t>(vals.cols()))), values_(std::move(vals)),
              map_(values_.data(), values_.rows(), values_.cols()) {
    }

    Dataset::Dataset(std::vector<std::string> const &vars, std::vector<std::vector<Turingforge::Scalar>> const &vals)
            : values_(MatrixFromValues(vals)), map_(values_.data(), values_.rows(), values_.cols()) {
        Hasher hasher;
    ...
    Dataset::Dataset(Matrix::Scalar const *data, Eigen::Index rows, Eigen::Index cols) // NOLINT
            : variables_(DefaultVariables(static_cast<size_t>(cols))), map_(data, rows, cols) {
    }

    void Dataset::SetVariableNames(std::vector<std::string> const &names) {
        if (names.size() != static_cast<size_t>(map_.cols())) {
            auto msg = fmt::format("The number of columns ({}) does not match the number of column names ({}).",
                                   map_.cols(), names.size());
    ...
    auto Dataset::VariableNames() const -> std::vector<std::string> {
        ...
        std::transform(variables_.begin(), variables_.end(), std::back_inserter(names),
                       [](auto const &p) { return p.second.Name; });
        return names;
    }
    ...
    auto Dataset::VariableHashes() const -> std::vector<Turingforge::Hash> {
        ...
        std::transform(variables_.begin(), variables_.end(), std::back_inserter(hashes),
                       [](auto const &p) { return p.second.Hash; });
        return hashes;
    }
    ...
    auto Dataset::VariableIndices() const -> std::vector<std::size_t> {
        ...
        std::transform(variables_.begin(), variables_.end(), std::back_inserter(indices),
                       [](auto const &p) { return p.second.Index; });
        return indices;
    }
