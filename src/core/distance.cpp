...
namespace Turingforge::Distance {
    namespace detail {
        ...
        template<typename T>
        inline auto CountIntersect(Turingforge::Span<T const> lhs, Turingforge::Span<T const> rhs) noexcept -> size_t
        {
        ...
        inline auto CountIntersect(Container const& lhs, Container const& rhs) noexcept -> size_t
        {
            ...
            return CountIntersect(Turingforge::Span<T const>(lhs.data(), lhs.size()), Turingforge::Span<T const>(rhs.data(), rhs.size()));
        }
    } // namespace detail
